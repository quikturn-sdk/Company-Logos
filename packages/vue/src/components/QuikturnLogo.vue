...
<template>
  <a
    ...
    :style="props.style"
  >
    <img :src="src" :alt="altText" :loading="loading" referrerpolicy="origin" @error="emit('error', $event)" @load="emit('load', $event)" />
  </a>
  <span v-else-if="props.class || props.style" :class="props.class" :style="props.style">
    <img :src="src" :alt="altText" :loading="loading" referrerpolicy="origin" @error="emit('error', $event)" @load="emit('load', $event)" />
  </span>
  <img v-else :src="src" :alt="altText" :loading="loading" referrerpolicy="origin" @error="emit('error', $event)" @load="emit('load', $event)" />
</template>
